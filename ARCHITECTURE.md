...
Save envelopes are version 4. Loading version 1/2/3 adds safe agriculture, weather, food-expiry, mood/productivity, food-point and neutral ownership defaults before normal validation. Job state remains ephemeral across loading, so reservations and carried ingredients are reconstructed safely.

Raw food items retain a physical stack with `freshPoints` and `spoiledPoints`; meals remain whole items worth 80 points. Cooking stations own a transient `ingredientFresh` buffer and `cookingProgress` while reserved by one pawn. Interrupted cooking refunds buffered fresh points as physical food. Spoiled points are separated into physical `waste` items capped at 25, which remain solid navigation obstacles. Spoilage uses indoor shelter, exposed weather, and a bounded 8-neighbour waste-contamination multiplier.
